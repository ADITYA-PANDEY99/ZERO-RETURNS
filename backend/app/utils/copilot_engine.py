...
"""
    ...
    def generate_sql(self, question: str) -> str:
        ...
        # Verify SQL safety
        from app.utils.security import is_sql_query_safe
        is_safe, reason = is_sql_query_safe(sql)
        if not is_safe:
            import logging
            logging.getLogger(__name__).warning(f"SECURITY: Blocked unsafe SQL query generated by LLM: '{sql}'. Reason: {reason}")
...
class AnalyticsCopilot:
    ...
    def ask(self, db_conn: sqlite3.Connection, query: str) -> Dict[str, Any]:
        ...
        if check_prompt_injection(query):
            return {
                "query": query,
                "business_summary": "I cannot process this request as it contains phrasing that violates system security guidelines.",
                "rag_hits": [],
                "sql_details": None,
                "charts_referenced": [],
                "recommendations": ["Refrain from using injection keywords."],
                "expected_impact": "Zero security incidents."
            }
